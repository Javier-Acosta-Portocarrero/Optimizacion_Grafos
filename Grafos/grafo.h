...
// unsigned es un numero natural

typedef struct
{
	unsigned j; // El nodo sucesor, predecesor o adyacente segun lo que vayamos a almacenar.
...
class GRAFO
 {
	...
	vector<LA_nodo>     LP;        // Lista de adyacencia de los predecesores: es una matriz donde cada fila posiblemente es posiblemente de distinto tamano
	vector<LA_nodo>     A;         // Matriz de adyacencia, tanto adyacencia como costes (grafo no dirigido) no la estamos usando
	// vector<LA_nodo>     LA;  Esto seria la lista de adyacencia de un grafo no dirigido.
...
// Para acceder seria LS[0][0].j    El primer nodo(j) del primer elemento  
#endif
